...
def fragment_eic(peaks, exmz, exrt, fragmz, mztol=0.05, rtlength=30):
    precursors = []
    for p in peaks:
        try:
            precursors.append(p.getPrecursors()[0].getMZ())
        except:
            pass
    precursors = np.unique(precursors)
    ...
    for p in peaks:
        if (p.getRT() >= rtrange[0]) and (p.getRT() <= rtrange[1]) and (p.getMSLevel()==2):
